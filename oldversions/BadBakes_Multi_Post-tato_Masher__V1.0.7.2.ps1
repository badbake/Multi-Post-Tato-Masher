...
$WindowTitle = "Badbakes_Multi Post-tato_Masher__V1.0.8"
...
function RunInstance {
	...
	# Write a message indicating that service.exe for the instance is starting, output to log.
    Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName is starting service.exe"
	"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName is starting service.exe" | Out-File -Append -FilePath $logFilePath

	# Start the service.exe process with the current set of arguments
	...
	# Get the process ID of the newly started service
	$servicePID = $serviceProcess.Id

	# Check if the service process has started successfully by checking for its PID
	if ($serviceProcess -ne $null -and (Get-Process | Where-Object { $_.Id -eq $servicePID })) {
		Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName has successfully started Post Service."
		"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName has successfully started Post Service." | Out-File -Append -FilePath $logFilePath
	} else {
		Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName has failed to start Post Service."
		"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $instanceName has failed to start Post Service." | Out-File -Append -FilePath $logFilePath
	}

	# Flag to track the previous state
    $previousState = ""
	
	# Run an initial grpcUrlCommand
	$response = (Invoke-Expression ("$($grpcurl) --plaintext -d '{}' localhost:$port spacemesh.v1.PostInfoService.PostStates 2>&1"))
	
	# Display initial grpCurl response to user and log to file.
	Write-Host "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $response"
	"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $response" | Out-File -Append -FilePath $logFilePath

    do {
        # Sleep for a short duration before the next status check
...
# Define a function to wait for user input (spacebar by default) to trigger all instances
function WaitForTriggerInput {
    Write-Host "Press spacebar to start all instances..."
    do {
        $key = $host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown").VirtualKeyCode
    } while ($key -ne 32)  # 32 is the virtual key code for spacebar
    # Trigger the RunAllInstances function once spacebar is pressed
    RunAllInstances
}
