...
.DESCRIPTION
    This script runs different instances of the PoST Proving "post-service.exe" sequentially, waits for each to complete before starting the next, and handles Cycle Gap timing.
.NOTES
...
function Run-Instance {
    ...
    Log-Message "$instanceName is starting post-service.exe" "INFO"
    $serviceProcess = Start-Process -FilePath ".\post-service.exe" -ArgumentList $arguments -NoNewWindow -PassThru -RedirectStandardError $serviceLogFilePath

    if ($serviceProcess -ne $null -and (Get-Process -Id $serviceProcess.Id -ErrorAction Inquire)) {
        Log-Message "$instanceName has started post-service.exe" "INFO"
    } else {
