...
function Run-Instance {
    ...
    $serviceProcess = Start-Process -FilePath $service -ArgumentList $arguments -NoNewWindow -PassThru -RedirectStandardError $serviceLogFilePath
...
function Run-AllInstances {
    # Sort the instance names to ensure they run in the same order every time
...
function Check-And-Run-ProvingInstances {
    ...
    if ($clearServiceLogFiles) {
        Clear-ServiceLogFiles -logDirectory $logDirectory -instances $sortedInstanceNames
    }
}
