...
function Run-AllInstances {
    ...
    Log-Message "All instances have completed."
    
    # Resume console update job after running instances
    Resume-ConsoleUpdateJob
}

# Function to calculate the next trigger time
...
# Function to wait for the timer trigger
...
# Function to wait for user input to trigger all instances manually
function Wait-ForTriggerInput {
    while ($true) {
        Write-Host "Press spacebar to start all instances manually or wait for the timer..."
...
# Main entry point
# Start the timer-based trigger and wait for user input simultaneously
Start-Job -ScriptBlock { Wait-ForTrigger }
